...
void configAP(void);
//void taskVerificacao(void *param);
...
void setup(void) {
  ...
  if(!bme.begin())
  {
    ...
    Serial.print("Endereço é: 0x"); Serial.println(bme.sensorID(), 16);
  ...
  if(WiFi.status() == WL_CONNECTED)
  {
    ...
    setupRTC();
    conectaMQTT();
    //xTaskCreatePinnedToCore(taskVerificacao, "TaskVerificacao", 10000, NULL, 2, NULL, 0);
...
void conectaMQTT(void)
{
  ...
  {
    if(client.connect("JardimVertical"))
    {
...
void pegaValores(float *p_temp, float *p_umid, float *p_press, float *p_umid_solo)
{
  ...
  *p_umid_solo = map(analogRead(34), 0, 4096, 100, 0);
}
...
void displayInfo(float *p_temp, float *p_umid, float *p_press, unsigned long *p_totalMilliLitres, float *p_flowRate, float *p_umid_solo, bool *p_solenoide, byte *p_hora, byte *p_minuto, byte *p_segundo, char **p_dados)
{
  if(WiFi.status() == WL_CONNECTED)
  {
    ...
    int tamanho_string = snprintf(NULL, 0, "Temperatura: %.2f°C | Umidade: %.2f%% | Pressao: %.2fhPa | Vazao: %.2fL/min | Quantidade Total de agua: %imL / %iL | Umidade do solo: %.2f%% | Horario dos dados: %d:%d:%d | Solenoide: %s\0", *p_temp, *p_umid, *p_press, *p_flowRate, *p_totalMilliLitres, *p_totalMilliLitres/1000, *p_umid_solo, data.hora, data.minuto, data.segundo, *p_solenoide ? "Ligada" : "Desligada");

    *p_dados = (char*)malloc((tamanho_string + 1) * sizeof(char));

    if(*p_dados != NULL)
    {
      sprintf(*p_dados, "Temperatura: %.2f°C | Umidade: %.2f%% | Pressao: %.2fhPa | Vazao: %.2fL/min | Quantidade Total de agua: %imL / %iL | Umidade do solo: %.2f%% | Horario dos dados: %d:%d:%d | Solenoide: %s\0", *p_temp, *p_umid, *p_press, *p_flowRate, *p_totalMilliLitres, *p_totalMilliLitres/1000, *p_umid_solo, data.hora, data.minuto, data.segundo, *p_solenoide ? "Ligada" : "Desligada");
    ...
    int tamanho_string = snprintf(NULL, 0, "Temperatura: %.2f°C | Umidade: %.2f%% | Pressao: %.2fhPa | Vazao: %.2fL/min | Quantidade Total de agua: %imL / %iL | Umidade do solo: %.2f%% | Horario dos dados: %d:%d:%d | Solenoide: %s\0", *p_temp, *p_umid, *p_press, *p_flowRate, *p_totalMilliLitres, *p_totalMilliLitres/1000, *p_umid_solo, *p_hora, *p_minuto, *p_segundo, *p_solenoide ? "Ligada" : "Desligada");
...
void enviaDadosMQTT(char **p_dados)
{
  client.publish("Dados_JardimVertical", *p_dados);
  Serial.println("Dados Enviados por MQTT");
}
